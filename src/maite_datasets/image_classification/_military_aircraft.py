...
_IMAGE_SIZE = 640
# Crops are widened to at least this many pixels so tiny objects stay classifiable.
_MIN_CROP = 128
# Index of the extra "background" class appended to the 88 aircraft classes.
_BACKGROUND_LABEL = 88


def _crop_chw(image: NDArray[Any], box: tuple[int, ...]) -> NDArray[Any]:
    """Crop a (C, H, W) image to `box` given as (x0, y0, x1, y1)."""
    x0, y0, x1, y1 = box
    return image[:, y0:y1, x0:x1]


class MilitaryAircraft(BaseICDataset[NumpyArray], BaseDatasetNumpyMixin):
    """
    The image classification version of a dataset that focuses on identifying different types of military aircraft.
    ...
    The dataset has 88 aircraft classes (plus 1 background class) from 4 overarching military aircraft categories.
    All images are at least 128 x 128 up to 640 x 640.
    ...
    def _load_data_inner(self) -> tuple[list[str], list[int], dict[str, Any]]:
        ...
        Each annotated object becomes its own classification sample: the datum reuses
        the source image path and carries the crop box that isolates that object.
        Images with an empty annotation file become a single "background" sample.
        """
        folders = ["train", "val", "test"] if self.image_set == "base" else [self.image_set]
        data_folder: list[Path] = []
        for folder in folders:
            data_folder.extend(sorted((self.path / folder).glob("*.txt")))
        if not data_folder:
        ...
        for file in data_folder:
            ...
            with open(file) as f:
                for line in f:
                    ...
                    xcenter, ycenter, width, height = (float(value) * _IMAGE_SIZE for value in out[1:])
                    self._crop.append(self._get_image_crop(xcenter, ycenter, width, height))
    ...
    def _get_image_crop(self, xcenter: float, ycenter: float, width: float, height: float) -> tuple[int, ...]:
        """Crop box around one object, widened to `_MIN_CROP` and shifted inside the image."""
        width = min(max(width, _MIN_CROP), _IMAGE_SIZE)
        height = min(max(height, _MIN_CROP), _IMAGE_SIZE)
        # Slide (rather than clamp) the box back in bounds so it keeps its size.
        x0 = min(max(xcenter - width / 2, 0), _IMAGE_SIZE - width)
        y0 = min(max(ycenter - height / 2, 0), _IMAGE_SIZE - height)
    ...
    def __getitem__(self, index: int) -> tuple[NumpyArray, NumpyArray, DatumMetadata]:
        ...
        # Get the image, cropped to this datum's object (deferred when lazy).
        # Background-only data points carry no crop and use the whole image.
        crop = self._crop[index]
        img = self._get_image(self._filepaths[index], [partial(_crop_chw, box=crop)] if crop else None)
