...
def disk(radius:int, alias_blur:float=0.1, dtype:np.dtype=np.float32) -> cv2.typing.MatLike:
    if radius <= 8:
        L = np.arange(-8, 8 + 1)
        ksize = (3, 3)
    else:
...
def clipped_zoom(x:NDArray, zoom_factor:float) -> NDArray[np.float32]:
    h = x.shape[1]
    # ceil crop height(= crop width)
    ch = int(np.ceil(h / float(zoom_factor)))

    top = (h - ch) // 2
    x = scizoom(x[:, top:top + ch, top:top + ch], (1, zoom_factor, zoom_factor), order=1)
...
def gaussian_blur(x:NDArray[np.number], severity:int=2) -> NDArray[np.float32]:
    ...
    x = np.array([gaussian(img / 255., sigma=c) for img in x])
    x = np.clip(x, 0, 1) * 255
    return x.astype(np.float32)
...
def glass_blur(x:NDArray[np.number], severity:int=1) -> NDArray[np.float32]:
    ...
    x = np.array([gaussian(img / 255., sigma=c[0]) for img in x]) * 255
    x = x.astype(np.uint8)
    ...
    x = np.clip(np.array([gaussian(img / 255., sigma=c[0]) for img in x]), 0, 1) * 255
    return x.astype(np.float32)
...
def defocus_blur(x:NDArray[np.number], severity:int=1) -> NDArray[np.float32]:
    ...
    out = np.array([cv2.filter2D(img, -1, kernel) for img in x])
...
def jpeg_compression(x:NDArray[np.number], severity:int=5) -> NDArray[np.float32]:
    ...
    def compress_one(img_arr: NDArray[np.uint8], quality:int) -> NDArray[np.number]:
        ...
        jpeg = PILImage.open(output)
        return np.array(jpeg)
    
    out = [compress_one(img, c) for img in x]
...
def pixelate(x:NDArray[np.number], severity:int=3) -> NDArray[np.float32]:
    ...
    def pixelate_one(img_arr: NDArray[np.uint8], size:int) -> NDArray[np.number]:
        img = PILImage.fromarray(img_arr, mode='L')
        img = img.resize((int(28 * size), int(28 * size)), PILImage.BOX)
        img = img.resize((28, 28), PILImage.BOX)
        return np.array(img)
    
    out = [pixelate_one(img, c) for img in x]
    out_arr = np.array(out)
...
def elastic_transform(x_arr:NDArray[np.number], severity:int=1) -> NDArray[np.float32]:
    ...
    x_arr = x_arr.astype(np.float32) / 255.
    shape = (x_arr.shape[-2], x_arr.shape[-1])
    ...
    def elastic_one(img:NDArray, M:NDArray, shape:tuple[int,int], indices:NDArray) -> NDArray[np.number]:
        img = cv2.warpAffine(img, M, shape, borderMode=cv2.BORDER_CONSTANT)
        return np.clip(map_coordinates(img, indices, order=1, mode='constant').reshape(shape), 0, 1)

    out = np.array([elastic_one(img, M, shape, indices) for img in x_arr]) * 255
    return out.astype(np.float32)
